...
<p>The listing is scheduled for {{ $listing->scheduled_for->setTimezone($actor->timezone) }}.</p>
...
<ul>
    ...
    <li>Date to remove the listing: {{ $listing->scheduled_until->setTimezone($actor->timezone) }}</li>
    @endif

    @if($listing->claimable_at)
    <li>Start date for claiming: {{ $listing->claimable_at->setTimezone($actor->timezone) }}</li>
    @endif

    @if($listing->claimable_until)
    <li>Last date for claiming: {{ $listing->claimable_until->setTimezone($actor->timezone) }}</li>
    @endif
</ul>
