...
class ItemWasClaimedEventHandler
{
    ...
    public function handle(NewItemWasClaimedEvent $event)
    {
        ...
        $inventoryItem = $claim->inventoryItem;
        $claimedBy = $claim->claimedBy;
        $seller = $inventoryItem->owner;
        $availableQty = $inventoryItem->getAvailableQuantity();
...
//        }
        ...
        try {
            ...
            if ($claimedBy->primaryEmail && ($claimedBy->primaryEmail->isVerified() || $claimedBy->isGuest())) {
                with(new KitEmail('inventory.claims.emails.claimed_toclaimer', ['item' => $inventoryItem], function ($mailer) use ($claimerEmail) {
            ...
            if ($seller->checkIsNotifyable('inventory_claimed', 'email')) {
                if ($seller->primaryEmail && $seller->primaryEmail->isVerified()) {
                    $this->toEmail($seller, $inventoryItem);
                }
            }
...
//            }
            ...
            $this->toDatabase($seller, $claim, $inventoryItem);
     ...
     */
    public function toEmail(User $seller, Inventory $inventoryItem)
    {
        $email = new KitEmail;
     ...
     * @param Inventory $inventoryItem
     */
    public function toWeb(User $user, Claims $claim, Inventory $inventoryItem)
    {
